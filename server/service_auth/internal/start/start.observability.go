...
func startMetricsServer(port int, path string) {
	...
	global.Logger.Info("Starting metrics server", "address", addr, "path", path)
	
	if err := http.ListenAndServe(addr, mux); err != nil {
		global.Logger.Error("Metrics server error", "error", err.Error())
	}
}
...
func ShutdownObservability() {
	if tracerProvider != nil {
		if err := tracerProvider.Shutdown(context.Background()); err != nil {
			global.Logger.Error("Error shutting down tracer", "error", err.Error())
		}
	}
}
